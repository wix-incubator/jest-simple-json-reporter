...
import { ciEnv, installSledLocalRequirements } from './utils'
...
test('single test - fail -> pass -> skip', async t => {
  const generateProject = async ({ test1_1Pass }: { test1_1Pass: boolean }) => {
    const result = await createFolderStrucutre({
      ...
      content: {
        'package.json': {
          ...
          license: 'MIT',
          scripts: {
            test: `${t.context.bin.testRetryPath} --test-runner sled-local -- ${t.context.bin.sledPath} local`,
          },
        },
        'pom.xml': `
    ...
    await installSledLocalRequirements({ cwd: result.entryPath })
