...
test('should not enable the feature - compilcated user command', async t => {
  const generateProject = () =>
    createFolder({
      'package.json': {
        name: 'test-project',
        license: 'MIT',
        scripts: {
          test: `TEST_1_1_PASS=false ${t.context.bin.testRetryPath} --test-runner jest -- echo 1 \\&\\& ${t.context.bin.jestPath}`,
        },
        jest: {
          reporters: ['default', jestSimpleJsonReporterPath],
        },
